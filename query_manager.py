...
class Determine_querry_type:
    ...
    def determine_query_type(self, user_query):
        ...
        # Combine table names and column names for matching
        schema_tokens = set()
        for name in table_names:
            schema_tokens.update(name.split('_'))  # Split table names by underscores
        for name in column_names:
            schema_tokens.update(name.split('_'))
        
        # Check for matches (fuzzy, substring, or token overlap)
        if any(token in schema_tokens for token in query_tokens) or \
           is_fuzzy_match(user_query_lower, table_names) or \
